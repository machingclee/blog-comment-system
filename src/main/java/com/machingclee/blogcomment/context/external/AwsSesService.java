package com.machingclee.blogcomment.context.external;
 ...
 * (e.g. notify comment thread participants of a reply).
 * <p>
 * Requires a verified SES identity for {@code app.ses.from-address}
 * (email identity or domain identity such as {@code machingclee.com}).
 * While the account is in the SES sandbox, every To/CC recipient must also be verified.
 * <p>
 * Optional {@code app.ses.cc-addresses} (comma-separated) are always CC'd on every send.
 */
@Service
...
public class AwsSesService {
     ...
     * (thread participants who already receive the email as To/BCC).
     *
     * @param to           primary recipient
     * @param bccAddresses additional blind-carbon-copy recipients (nullable)
     * @param ccExclusions addresses to omit from the configured CC list (nullable)
     * @return SES message id
     */
    public String send(String to, String subject, String textBody, String htmlBody,
     ...
     * (e.g. thread participants already receiving the mail).
     */
